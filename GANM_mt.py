...
def Main(file_name,aj_instance_name):
	...
	GA_p(agents)
def GA_p(agents,num_exchange_round=100,num_rounds=20,num_parent=50):
	global AsP_popu
	AsP_popu=[agents[i].GANM_A_propose_Cmax(num_parent,num_multi=3) for i in range(len(agents))]
	for _i in range(num_rounds):
		myt=[]
		for i in range(len(agents)):
			t_1=threading.Thread(target=GA_agent,args=(agents[i],i,AsP_popu[i],num_exchange_round,num_parent))
			myt.append(t_1)
		for t in myt:
			t.start()
		for t in myt:
			t.join()
		Apopu_exchange(agents)
		print('kakkakkakakakkakakakka',_i)
	print(AsP_popu)
def GA_agent(agent,i,P_popu,num_exchange_round,num_parent):
	global AsP_popu
	for r in range(num_exchange_round):
		...
		# print('tread ',i,'round ',r)
	AsP_popu[i]=P_popu
def Apopu_exchange(agents,np_exchange=5):
	global AsP_popu
